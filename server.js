...
setInterval(() => {
  http.get(`https://gamsa-bot.glitch.me/`);
}, 280000);
...
const youtube = new YouTube("AIzaSyDzMBqnavrrXuu-WPP06dUnZ-LQRAiaUeM"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDzMBqnavrrXuu-WPP06dUnZ-LQRAiaUeM"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@554963267727130625>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (message.content === "كسمك") {
    message.channel.send("**:heart:كسمين امك يبن البتناكه:heart:**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content === "هلا") {
    message.channel.send("**:heart:هلا والله:heart:**");
    message.channel.sendFile("");
  }
});
...
client.on("message", async message => {
  if (message.content.includes("discord.gg")){
    if (message.membre.haspermission("MANAGE_GUILD")) return;
    if(!message.channel.guild) return;
    message.delate();
  }
});
